...
function parsePixelWidths(source) {
  ...
  if (trimmed.includes(",")) {
    return trimmed.split(",").map((value) => value.trim()).filter(Boolean).map((value) => Number.parseInt(value, 10));
  }
  return Array.from(
    trimmed.replace(/\s/g, ""),
...
var WebGLPixelTextMaterial = class extends ShaderMaterial {
  constructor(settings) {
    const uniforms = {
      alignment: new Uniform(new Vector23(settings.align, -settings.vAlign)),
      color: new Uniform(new Color2(settings.color)),
      fontSizeInChars: new Uniform(new Vector23(1, 1)),
      fontTexture: new Uniform(
        settings.fontFace.texture ?? getFallbackTexture()
      ),
      layoutSizeInCharColumns: new Uniform(new Vector23(1, 1)),
      layoutSizeInChars: new Uniform(new Vector23(1, 1)),
      layoutTexture: new Uniform(getFallbackTexture()),
      strokeColor: new Uniform(new Color2(settings.strokeColor))
    };
    if (settings.screenSpace) {
      ...
      uniforms.clipSpacePosition = new Uniform(new Vector42());
...
var PixelTextMesh = class extends PixelTextMeshBase {
...
import { Color as Color3, Vector2 as Vector24, Vector4 as Vector43 } from "three";
...
var WebGPUPixelTextMaterial = class extends NodeMaterial {
  constructor(settings) {
    super();
    __publicField(this, "clipSpacePosition", new Vector43());
    __publicField(this, "fontSizeInChars", new Vector24(1, 1));
    __publicField(this, "layoutSizeInChars", new Vector24(1, 1));
    __publicField(this, "layoutSizeInCharColumns", new Vector24(1, 1));
    ...
    const colorNode = uniform(new Color3(settings.color), "color");
    const strokeColorNode = uniform(
      new Color3(settings.strokeColor),
      "color"
    );
    ...
    const alignmentNode = uniform(
      new Vector24(settings.align, -settings.vAlign),
      "vec2"
    );
    if (settings.screenSpace) {
      if (!settings.pixelSizeInClipSpaceUniform) {
...
var src_default = {
  ...
  pixelFontFaces,
  pixelTextSettings,
  WebGLPixelTextMesh: PixelTextMesh,
  WebGPUPixelTextMesh
};
export {
  PixelFontFace,
  PixelTextMesh,
  PixelTextMesh as WebGLPixelTextMesh,
  WebGPUPixelTextMesh,
